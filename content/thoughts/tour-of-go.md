...
title: Tour of Go
created: 2026-07-24
modified: 2026-08-02T04:50:59.355+05:30
tags:
  - learning
  - go
---
...
### **How does Go decide what's "public" and what's "private"?**
...
### **Why does Go write `func add(x int, y int) int` instead of `int add(int x, int y)` like C?**
...
### **Why can Go functions return more than one value?**
...
### **What happens if you declare a variable and never assign it anything?**
...
### **Why won't Go let you mix an `int` and a `float64` without casting?**
...
### **Then why do constants behave differently?**
...
### **What are the actual basic types Go gives you to work with?**
...
### **Where's `while` and `do-while` in Go?**
...
### **Why doesn't Go's `switch` fall through to the next case automatically, like C's does?**
...
### **What does `defer` actually buy you?**
...
### **What's a struct, at its simplest?**
...
### **Can I take a pointer to a struct field and modify it directly?**
...
### **If I have a pointer to a struct, do I have to write `(*p).Name` every time I want a field?**
...
### **What's actually different between `[5]int` and `[]int`?**
...
### **If I pass a slice into a function and modify an element, does the caller see that change?**
...
### **Why does `append` sometimes jump capacity in weird amounts, like from 2 to 6, not straight to what you need?**
...
### **Why can't I do 2D slices in one line, the way I'd declare a 2D array in C?**
...
### **What happens if I try to write to a map I never initialized?**
...
### **How do I check if a key exists without a value being ambiguous (e.g. is `0` a real value or a missing one)?**
...
### **What's actually happening when a function returns another function?**
...
### **Why does `a, b = b, a+b` matter for something like a Fibonacci generator?**
...
### **What's a "method" in Go, really, is it different from a function?**
...
### **If a method has a pointer receiver, do I need to manually take the address every time I call it?**
...
### **So when should I actually choose a pointer receiver over a value receiver?**
...
### **How does a type "implement" an interface in Go, where's the `implements` keyword?**
...
### **Does having extra methods beyond what the interface needs disqualify a type?**
...
### **This is the trickiest part of Go, why can `err != nil` be true even when `err` looks like it should be nil?**
...
### **How do I get the concrete type back out of an interface?**
...
### **What's `Stringer`, and why does `fmt.Println` sometimes print something custom?**
...
### **What's the classic trap when writing a custom `error` type?**
...
### **Why do most custom error types use a pointer receiver instead of a value receiver?**
...
### **With `io.Reader`, why does `Read` return an integer, isn't that awkward compared to just returning the data?**
...
### **What is the "empty interface" (`interface{}`), and why does it feel like it can hold anything?**
...
### **How does `image.Image` let you generate a picture out of nothing but code?**
...
### **Why does that `comparable` word matter, can't `T` just be anything?**
...
### **When I write methods on a generic struct, do I have to repeat the constraint every time?**
...
### **What actually is a goroutine, a thread?**
...
### **How does Go run a huge number of goroutines on just a handful of CPU cores?**
...
### **What's the actual philosophy behind channels?**
...
### **How does a receiver know when a sender is completely done sending, rather than just pausing?**
...
### **What does `select` do that a regular `if` can't?**
...
### **If channels handle communication, why does Go still have `sync.Mutex`?**
